...
const BALLS_PER_WEEK = 6;

function randomDraw() {
  const nums = [];
  while (nums.length < BALLS_PER_WEEK) {
...
export default function App() {
  ...
  const totalPot = useMemo(() => {
    const paid = players.reduce(
      (sum, p) => sum + getWeeksPaid(p, club.week, club.week_drawn) * WEEKLY_FEE,
      0
    );
  ...
  const winnerPrizeEach =
    club.winner_names.length > 0
      ? totalPot / club.winner_names.length
      : totalPot;
  ...
  async function addPlayer(e) {
    ...
    if (players.length >= MAX_PLAYERS) {
      alert(`Club is full (${MAX_PLAYERS} players max)`);
      return;
    }
    ...
    if (numbers.length !== NUMBERS_PER_PLAYER) {
      alert(`Enter exactly ${NUMBERS_PER_PLAYER} numbers`);
      return;
    }

    const unique = [...new Set(numbers)].sort((a, b) => a - b);

    if (unique.length !== NUMBERS_PER_PLAYER) {
      alert(`Numbers must be ${NUMBERS_PER_PLAYER} different numbers`);
      return;
    }

    if (unique.some((n) => n < 1 || n > MAX_NUMBER)) {
      alert(`Numbers must be between 1 and ${MAX_NUMBER}`);
      return;
    }
  ...
  async function drawNumbers() {
    ...
    setTimeout(async () => {
      ...
      setClub((prev) => ({ ...prev, week_drawn: true }));
  ...
  async function finishWeekCheck(updatedDrawn, drawWeek) {
    ...
    const winners = activePlayers.filter((p) => {
      ...
      return hits.length === NUMBERS_PER_PLAYER;
    });
    ...
    if (winners.length > 0) {
      ...
      const potAtWin =
        players.reduce(
          (sum, p) => sum + getWeeksPaid(p, drawWeek, true) * WEEKLY_FEE,
          0
        ) + Number(club.carryover || 0);

      if (winnerList.length === 1) {
        alert(`Winner: ${winnerList[0]} — prize ${formatMoney(potAtWin)}`);
      } else {
        alert(
          `Winners: ${winnerList.join(", ")}\nEach wins ${formatMoney(
            potAtWin / winnerList.length
          )}`
        );
      }
    }
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        >
          ...
          >
            ...
            />
            <div>
              <h1 style={{ margin: 0, fontSize: 36 }}>Private Friends Bingo Club</h1>
              <div style={{ marginTop: 6, opacity: 0.95 }}>
                {formatMoney(WEEKLY_FEE)} weekly • {NUMBERS_PER_PLAYER} numbers • {BALLS_PER_WEEK} balls drawn weekly
              </div>
              <div style={{ marginTop: 6, opacity: 0.95 }}>
                Numbers 1–{MAX_NUMBER} • Private club • Max {MAX_PLAYERS} players
              </div>
            </div>
          </div>
        ...
        <div style={statsGrid}>
          ...
          />
          <StatCard
            title={club.winner_names.length > 1 ? "Prize Each" : "Winner Prize"}
            value={formatMoney(club.winner_names.length > 1 ? winnerPrizeEach : totalPot)}
        ...
        >
          ...
          <p style={{ marginBottom: 0, lineHeight: 1.7 }}>
            This is a private invite-only bingo club for friends. Each player picks {NUMBERS_PER_PLAYER} numbers
            from 1 to {MAX_NUMBER}. Each week exactly {BALLS_PER_WEEK} balls are drawn. Drawn numbers stay marked
            week to week until someone matches all {NUMBERS_PER_PLAYER} numbers. The first weekly draw where a player
            completes all {NUMBERS_PER_PLAYER} wins 100% of the jackpot. If multiple players complete all numbers
            in the same draw, the jackpot is split equally.
          </p>
        </div>
        ...
        <div style={{ ...card, background: "#ffffffee" }}>
          ...
          <p style={{ marginTop: 16 }}>
            ...
            {club.winner_found
              ? "Winner found"
              : club.week_drawn
              ? `Week ${club.week} complete — start next week when ready`
              : `Week ${club.week} open for its weekly draw`}
          </p>
          ...
          {club.winner_found && (
            ...
            >
              {club.winner_names.length === 1
                ? `Winner: ${club.winner_names[0]} — prize ${formatMoney(totalPot)}`
        ...
        <div style={twoCol}>
          <div style={{ ...card, background: "#fff7ed" }}>
            ...
            >
              <strong>Private invite-only club.</strong> Admin adds players manually.
              Maximum {MAX_PLAYERS} players. Each player chooses {NUMBERS_PER_PLAYER} numbers from 1 to {MAX_NUMBER}.
            </div>

            {players.length >= MAX_PLAYERS && (
              <p style={{ color: "#b91c1c", fontWeight: "bold" }}>
                Club is full ({MAX_PLAYERS} players)
              </p>
            )}
          ...
          <div style={{ ...card, background: "#f0fdf4" }}>
            <h2 style={{ marginTop: 0, color: "#166534" }}>Club Rules</h2>
            <ul style={{ margin: 0, paddingLeft: 18, lineHeight: 1.8 }}>
              <li>Private invite-only friends bingo club</li>
              <li>Maximum {MAX_PLAYERS} players</li>
              <li>{formatMoney(WEEKLY_FEE)} weekly contribution per active player</li>
              <li>Each player chooses {NUMBERS_PER_PLAYER} numbers from 1 to {MAX_NUMBER}</li>
              <li>Exactly {BALLS_PER_WEEK} balls are drawn each week</li>
              <li>Drawn numbers remain marked week to week</li>
              <li>First player to match all {NUMBERS_PER_PLAYER} numbers wins 100% of the jackpot</li>
              <li>If multiple players match all numbers in the same draw, jackpot is split equally</li>
              <li>If nobody wins, the jackpot rolls over to the next week</li>
              <li>Only one weekly draw is allowed per week</li>
            </ul>
          </div>
        </div>
...
function PlayerCard({
  ...
  const totalPaid = paidWeeks * WEEKLY_FEE;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={pill}>{hits.length}/{NUMBERS_PER_PLAYER}</span>
      </div>
